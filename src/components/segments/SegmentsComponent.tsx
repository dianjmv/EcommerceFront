...
function Segments() {
    ...
    for (let segment of segments) {
        for (let level of segment.level) {
            const brands = levelsTemp.filter(
                levels => level.product_brand.id === levels.product_brand.id && levels.segmentId === segment.id
            );
            if (brands.length < 1) {
            ...
            <div className={'grid grid-cols-1 container mt-16'}>
                ...
                {segments.map((segment, index) => (
                    <div className={`grid md:grid-cols-3 grid-cols-1 bg-gray-200 ${index >= 1 ? 'my-1' : ''}`}>
                        <div
                            className={'text-white flex flex-wrap text-center content-center '}
                            style={{
                        ...
                        <div className={'flex flex-wrap text-center content-center'}>
                            <p className={'mx-auto px-16 py-10'}>{segment.description}</p>
                        </div>
                    </div>
                ))}
            </div>

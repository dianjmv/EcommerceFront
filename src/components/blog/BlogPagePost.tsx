...
export default function BlogPagePost(props: BlogPagePostProps) {
    ...
    function searchBlog(searchInput: ChangeEvent): void {
        // @ts-ignore
        const filteredPosts = posts.filter(post => (post.title.toLowerCase().includes(searchInput.target.value.toLowerCase())))
                        ...
                        <li key={post.id} className={'grid grid-cols-2 gap-x-2 my-4'}>
                            <div className={'col-start-1'}>
                                <AppLink href={`/blog/post/${post.slug}`}>
                                    <img src={`${process.env.NEXT_PUBLIC_BASE_URI}${post.thumbnails.url}`}
                                         alt=""/>
                                </AppLink>
                            </div>
        ...
        return (
            <div className={'w-full'}>
                ...
                <div className={'md:pt-4'}>
                    <input type="text" onChange={searchBlog} className={'w-full form-control'}
                           placeholder={'Buscar'}/>
                </div>
                <h3 className={'text-xl text-black border-gray-300 border-b-4 mt-4 mb-3'}>Posts
                    Recientes</h3>
                <ul>
                    {
                        postsFiltered.length > 0 ?
                            postRecently(postsFiltered) :
                            postRecently(posts)
                    }
                </ul>
            </div>

...
import { useShopFilterValues, useShopSetFilterValueThunk } from '../../store/shop/shopHooks';
...
function WidgetFilters(props: WidgetFiltersProps) {
        ...
        <div className="filter-price">
            <div className="filter-price__slider" dir="ltr">
                <InputRange
                    ...
                    value={{
                        min:
                            filtersAdded.filters.filter(filter => filter.type === 'price').length > 0
                                ? // @ts-ignore
                                  filtersAdded.filters.filter(filter => filter.type === 'price')[0].value.min
                                : minValue,
                        max:
            ...
            <div className="filter-price__title">
                ...
                <span className="filter-price__min-value">
                    <CurrencyFormat value={minValue} />
                </span>
                {' – '}
                <span className="filter-price__max-value">
                    <CurrencyFormat value={maxValue} />
                </span>
            </div>
        </div>
    );
                    ...
                    <div className="filter__container">
                        {filter.type === 'price'
                            ...
                            : filter.element.map(elemt => (
                                  <label key={elemt.slug} className={'filter-list__item'}>
                                      <span className="filter-list__input input-radio">
                                          <span className="input-radio__body">
                                              <input
                                                  ...
                                                  type="checkbox"
                                                  checked={
                                                      filtersAdded.filters.filter(
                                                          productFilter =>
                                                              productFilter.type === filter.type &&
                                                              productFilter.slug === elemt.slug
                                                      ).length > 0
                                                  }
                                                  name={elemt.slug}
                                                  disabled={
    ...
    return (
        <div className={classes}>
            ...
            <div className="widget-filters__actions d-flex mb-n2"></div>
        </div>
    );
}
